...
Ferramenta que gera pirâmides demográficas de população de software. Lê um
contrato de entrada padronizado e devolve tabelas e figuras. O dump MSR14 entra
por um adaptador e serve de caso de aceite, porque reproduz os números
publicados por Onoue et al. (ESEM14 e IEICE16).

Antes de qualquer coisa neste repo, ler:

- **`docs/ferramenta/FONTES.md`**: contrato de entrada, o que um adaptador precisa entregar.
- **`INSTRUCOES_CLAUDE_CODE.MD`**: spec de implementação do motor (fonte da verdade; vence sobre qualquer outra doc em caso de conflito).
